...
//根据不同的header去设置不同参数
var diffOptions = (parameter, type) => {
	var res = {
		json: {
			json: parameter,
		},
...
//post请求
var postFun = (url, parameter = {}, type = 'json') => {
	//判断根据不同的header去设置参数
	var res = request('POST', url, diffOptions(parameter, type))
...
//写入文件-isReduce:是否压缩
var writeFileSyncFun = function(body, cacheFile, isReduce) {
	if(isReduce) {
